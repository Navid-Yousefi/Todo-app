...
async def user_login(request: UserLoginSchema, db: Session = Depends(get_db)):
    ...
    token_obj = TokenModel(user_id=user_obj.id, token=generate_access_token(user_id=user_obj.id))
    db.add(token_obj)
    db.commit()
...
async def user_refresh_token(request: UserRefreshTokenSchama, db: Session = Depends(get_db)):
